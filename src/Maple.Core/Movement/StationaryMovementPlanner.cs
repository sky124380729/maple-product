...
public sealed class StationaryMovementPlanner(IRandomSource random)
{
    ...
    public MovementPlan CreatePlan(StationaryAttackConfig config)
    {
        ...
        int firstHold = SampleHold(firstDirection, RelativeOffsetMs, config, "INITIAL_FACING_BUDGET_EXHAUSTED");
        ...
        int secondHold = SampleHold(secondDirection, afterFirst, config, "MOVEMENT_BUDGET_EXHAUSTED");
