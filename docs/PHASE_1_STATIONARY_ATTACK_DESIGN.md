...
- `VisualStationaryProfile`：schema 1 保存旧版名字模板；schema 2 保存捕获宽高、黄色平台外框、蓝色人物外观源框和 1–8 个固定 BGRA 模板。平台和人物模板仍由同一文件原子持久化，但编辑生命周期相互独立；平台更新只替换 `Platform` 并完整复用人物模板库，人物更新只替换人物模板库并保留平台。人物模板库保存可选采集时间，旧 schema 2 缺少该字段时以档案更新时间显示。绿色安全核心只由保护带计算，不持久化为用户选择。
...
- `SelfAppearanceTemplateMatcher` 的黄色全域调用使用稀疏特征粗筛到完整特征精核：所有像素位置都参与粗筛，每张模板使用按强度排序的前 16 个边缘样本；随后分别在粗筛最佳空间峰和排除同一目标后的次佳空间峰周围 `2px` 内，用完整最多 128 个样本逐像素复核并重新排序。锚点局部调用直接使用完整样本。不得用空间步进网格、跳过空间第二候选或放宽歧义门槛。
...
配置入口位于独立原生预览。没有有效人物外观模板时，首次流程先由用户用黄色框选择平台外层安全范围，再用蓝框紧贴选择本人头部和上半身，不含名字、宠物和大面积武器/技能特效。已有有效人物模板且视口尺寸一致时，“配置平台”只重新选择黄色平台并立即保存，不进入人物框选或动作采集；“更新人物模板”只重新选择蓝框并采集动作，保留当前黄色平台。Host 自动显示绿色保护带内安全核心。绿色核心内允许双向随机移动，黄色外框与绿色核心之间只授权向内修正，黄色外框之外冻结移动。

视觉配置与跟踪覆盖层只创建有颜色语义的矩形和轻透明填充，不创建任何画面内文字控件；拖拽中的临时框同样无文字。用途图例、当前步骤、采集进度、错误和保存结果由画面外工具栏或底部状态栏承载。工具栏另显示 1–8 张已保存人物模板的真实缩略图、数量和采集时间，并提供独立更新入口；没有人物模板时隐藏缩略图区域。模板图像不得经 React bridge 发布。预览复用视觉攻击控制器使用的同一观察会话，在空闲时也持续处理新帧，并以青色可信框或橙色候选框显示当前最佳位置。UI 合并掉帧时必须捕获同一帧处理后生成的观察快照，禁止在旧位图上读取更新后的候选坐标。人物框在 1366 宽参考客户区不得小于 `24x32` 或大于 `112x144`，按实际客户区宽度等比缩放，并必须具有足够纹理。平台矩形和人物源位置是固定坐标；每次启动仍需重新连续锁定 3 帧。旧 schema 1 名字配置继续读取，但下一次平台配置需完成一次人物外观采集以升级。平台或人物保存失败、校准失败或用户按 Esc 取消时，磁盘和内存中的旧完整配置保持不变。预览工具栏的配置、更新和清除命令与主窗口使用同一生命周期门及删除路径；攻击或寻路的启动准备及运行期间均拒绝修改，成功清除后销毁空闲观察会话并清空全部覆盖框。
...
5. `UntrustedFrozen` 在连续 `15s` 内不发送左右键但继续随机攻击。可信移动按方向持续采样 `px/ms`，左右各 2 个有效样本只是像素预测启用门槛；会话内继续维护每侧最近 32 个有效样本的中位数。达到 `15s` 后，同一个 `VisualStationarySessionController` 优先启用 `VisualFallbackMovementPlanner`，每个随机按压候选同时预演真实时间 offset 和预测像素 offset 加不确定度，只有两者分别处于 `maxLateralMoveMs` 与平台可用像素边界内才可发送。像素预测尚未标定时才使用普通 `StationaryMovementPlanner`，并把按压候选限制在配置范围的下半段；没有安全候选时冻结本轮移动、继续攻击。回退规划器以当前真实时间模型和最后可信像素位置启动；若时间值已超出配置边界，只压到同符号的 `maxLateralMoveMs` 后继续，不得以 `MOVEMENT_OFFSET_EXCEEDED` 停止攻击。回退两段之间先固定等待 `100ms`，再完整等待一次配置范围内独立随机间隔；从首段开始到稳定等待完成保持人物动作跟踪窗口开启，使移动后重新连续 3 帧可信时能更新锚点。观察陈旧或捕获/预览故障也从首次连续不可用时刻累计 `15s`；schema 或视口结构错误在启动前拒绝。人物外观重新连续 3 帧可信后，在下一完整移动周期以最新真实像素位置和当前真实时间 offset 重新锚定，退出回退并恢复视觉授权。`OutsideFrozen` 永不回退盲走。人物外观首次锁定阈值为 `0.70`、空间次峰差值为 `0.06`；已有轨迹和局部恢复阈值为 `0.68`、局部次峰差值为 `0.04`，只接受各轴 `12px` 邻域并连续 3 帧恢复。
...
Host 向 React 只发布配置状态、运行安全状态、匹配分数和相对安全区中心的 `visualOffsetPx`。负数为左、正数为右。模板像素、平台坐标、随机候选和方向动作不能由 React 提交。结构化日志记录每个可信移动校准尝试及接受/拒绝原因、方向、动作前后视觉 X、位移、候选 `px/ms`、每侧样本数和滚动中位数；回退日志记录候选裁剪、预测前后像素 offset、不确定度、真实按压毫秒和时间模型 offset。日志保留全部拒绝样本供离线汇总，但本期不自动跨会话固化换算关系。
