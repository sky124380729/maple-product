...
- `leaseMs` 作为 Host 计划保持时长传给 Broker；Broker 记录活动键，并由 watchdog 负责断联、心跳和安全门兜底释放。物理硬截止调度器暂不作为一期已实现能力，避免短租约在高负载下把正常收尾误判为会话错误。
- Host 在计划保持时间结束后发送 `KeyUp`；`KEY_UP_SENT` 和幂等 `KEY_ALREADY_UP` 都是正常收尾结果。后续重新设计物理硬截止时，必须补充独立调度和迟到诊断测试。
