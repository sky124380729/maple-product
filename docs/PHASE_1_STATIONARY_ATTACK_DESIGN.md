...
- `SelfIdentityStabilizer`：人物外观首次锁定和轨迹完全重置后的重新锁定要求最佳分数至少 `0.70`、最佳与空间次佳差值至少 `0.06`、连续 3 个递增帧序列且中心跳变不超过 `12px`；旧 schema 1 名字模板继续使用既有阈值。已有可信外观轨迹时，候选只有位于当前搜索锚点水平和垂直各 `12px` 邻域内才使用 `0.68` 跟踪门槛，形成 `0.68` 跟踪、`0.70` 获取的滞回；短暂失信后仍需连续 3 个新帧恢复。较低跟踪门槛不得放宽首次锁定、最大跳变或局部次峰 `0.04` 差值要求。同一人物框中心相距不足半个模板宽且半个模板高的相邻峰合并为一个目标，不能制造虚假的次峰歧义；远处第二峰不覆盖当前轨迹，局部空间分离候选才视为歧义。
